<template>
  <div class="detail-backdrop">
    <div class="detail-container">
      ...
      <div class="genre-info-section">
        <div class="genre-info-card">
          <h3>Información del Género</h3>
          <div class="info-grid">
            <div class="info-item">
              <label>Código:</label>
              <span>{{ genre.codigo }}</span>
            </div>
            <div class="info-item">
              <label>Nombre:</label>
            ...
            <div class="info-item">
              ...
              <span>{{ formatDate(genre.fecha_creacion) }}</span>
            </div>
            <div class="info-item">
              <label>Estado:</label>
      ...
      <div class="books-section">
        ...
        <div v-if="!loadingBooks" class="books-list">
          <div v-if="genreBooks.length === 0" class="empty-books">
            <p>Este género no tiene libros asignados</p>
          </div>
          <div v-else class="books-grid">
            <div v-for="bookGenre in genreBooks" :key="bookGenre.id" class="book-card">
              ...
              <div class="book-actions">
                <button 
                  v-if="bookGenre.flag"
                  @click="confirmRemoveBook(bookGenre)" 
                  class="btn-remove-book"
                  title="Desactivar libro del género"
                >
                  Desactivar
                </button>
                <button 
                  v-else
                  @click="confirmReactivateBook(bookGenre)" 
                  class="btn-reactivate-book"
                  title="Reactivar libro del género"
                >
                  Reactivar
                </button>
              </div>
      ...
      <div v-if="showRemoveBookModal" class="modal-backdrop">
        <div class="modal delete-modal">
          ...
          <div class="modal-body">
            <p>¿Estás seguro de que deseas desactivar el libro del género?</p>
            <p class="delete-warning">La relación libro-género será marcada como inactiva.</p>
          </div>
          <div class="modal-footer">
            <button @click="showRemoveBookModal = false" class="btn-cancel">Cancelar</button>
            <button @click="removeBookFromGenre" class="btn-confirm-delete" :disabled="isRemovingBook">
      ...
      <div v-if="showReactivateBookModal" class="modal-backdrop">
        <div class="modal delete-modal">
          ...
          <div class="modal-body">
            <p>¿Estás seguro de que deseas reactivar el libro del género?</p>
          </div>
          <div class="modal-footer">
            <button @click="showReactivateBookModal = false" class="btn-cancel">Cancelar</button>
            <button @click="reactivateBookInGenre" class="btn-confirm-reactivate" :disabled="isReactivatingBook">
...
export default {
  ...
  methods: {
    ...
    async loadGenreBooks() {
      this.loadingBooks = true;
      
      try {
        // Cargar libros del género específico
        const response = await axios.get(
          `http://127.0.0.1:8001/lib/api/libro-genero/?genero=${this.genre.id}`, 
          { headers: this.getAuthHeaders() }
        );
        this.genreBooks = response.data;
      } catch (error) {
        console.error('Error al cargar libros del género:', error);
        this.showNotification('Error al cargar los libros del género', 'error');
      } finally {
    ...
    async loadAvailableBooks() {
      ...
      try {
        ...
        // Filtrar libros que ya están asignados al género (activos)
        const assignedBookIds = this.genreBooks
          .filter(bg => bg.flag)
          .map(bg => bg.libro);
    ...
    async addBookToGenre() {
      ...
      try {
        ...
        // Agregar el nuevo libro-género a la lista
        this.genreBooks.push(response.data);
        
        this.showNotification('Libro agregado correctamente al género', 'success');
        this.cancelAddBook();
        
      } catch (error) {
        console.error('Error al agregar libro al género:', error);
        
        let errorMessage = 'Error al agregar el libro al género';
    ...
    confirmRemoveBook(bookGenre) {
      this.bookToRemove = bookGenre;
      this.showRemoveBookModal = true;
    },

    confirmReactivateBook(bookGenre) {
      this.bookToReactivate = bookGenre;
      this.showReactivateBookModal = true;
    },

    async removeBookFromGenre() {
      ...
      try {
        ...
        await axios.put(
          `http://127.0.0.1:8001/lib/api/libro-genero/${this.bookToRemove.id}/`,
          updateData,
          { headers: this.getAuthHeaders() }
        );

        // Actualizar en la lista local
        const index = this.genreBooks.findIndex(bg => bg.id === this.bookToRemove.id);
        if (index !== -1) {
          this.genreBooks[index] = { ...this.genreBooks[index], flag: false };
        }

        this.showNotification('Libro desactivado del género correctamente', 'success');
        
      } catch (error) {
        console.error('Error al desactivar libro del género:', error);
        
        let errorMessage = 'Error al desactivar el libro del género';
        if (error.response?.status === 401) {
          errorMessage = 'No tienes autorización para realizar esta acción';
        }
    ...
    async reactivateBookInGenre() {
      ...
      try {
        ...
        await axios.put(
          `http://127.0.0.1:8001/lib/api/libro-genero/${this.bookToReactivate.id}/`,
          updateData,
          { headers: this.getAuthHeaders() }
        );

        // Actualizar en la lista local
        const index = this.genreBooks.findIndex(bg => bg.id === this.bookToReactivate.id);
        if (index !== -1) {
          this.genreBooks[index] = { ...this.genreBooks[index], flag: true };
        }

        this.showNotification('Libro reactivado en el género correctamente', 'success');
        
      } catch (error) {
        console.error('Error al reactivar libro en el género:', error);
        
        let errorMessage = 'Error al reactivar el libro en el género';
        if (error.response?.status === 401) {
          errorMessage = 'No tienes autorización para realizar esta acción';
        }
